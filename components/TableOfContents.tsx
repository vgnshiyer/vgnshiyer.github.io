    ...
    <div className='w-1/4 hidden lg:block ml-8 fixed top-0 right-0 p-4 mt-40'>
        ...
        <ul className='list-disc list-inside mt-2'>
            {headings.map(( { id, level, text }: any ) => (
              <li key={id} className={`text-tertiary-light dark:text-tertiary-dark ${getTextSize(level)} ml-${level * 2} mb-1 list-none hover:text-contrast-light dark:hover:text-contrast-dark`}>
                <a href={`#${id}`}>{text}</a>
              </li>
            ))}
        </ul>
